...
pub(crate) fn scan_duplicates(a: Weak<MainWindow>, sd: ScanData) {
    thread::Builder::new()
        ...
        .spawn(move || {
            ...
            sd.shared_models.write().expect("RwLock poisoned").shared_duplication_state = Some(tool);
...
fn write_duplicate_results(
    ...
    for (ref_fe, vec_fe) in vector.into_iter().rev() {
        ...
        if let Some(ref_fe) = ref_fe {
            ...
            // Update is_hardlinked on the header row
...
fn prepare_data_model_duplicates(fe: DuplicateEntry) -> (ModelRc<SharedString>, ModelRc<i32>) {
    ...
    let data_model_int_arr: [i32; MAX_INT_DATA_DUPLICATE_FILES] = [modification_split.0, modification_split.1, size_split.0, size_split.1, inode_split.0, inode_split.1];
